...
| `js/platform.js` | StarHermit adapter (`DWPlatform`): fragment launch token + Bearer + 45-min refresh, server time, profile nickname, score boards (its-backend), cloud-save slot (zip+base64, debounced), offline fallbacks |
...
**Used (hosted):** `platform.init` reads `#game_token=<jwt>` from the URL fragment (optional `&session_id=`, stripped after the read; `?launch=`/`?token=` kept for local dev), decodes the JWT **payload** segment for `sub` + `game_scope` (never hard-coded; the old header-segment/`json.scope` decode is fixed), sends the token as `Authorization: Bearer` on same-origin `/api/v1` calls, re-mints it every 45 min via `POST /api/v1/games/{slug}/launch-token` (60 s retry), and never stores it. The display name is the profile nickname from `GET /api/v1/users/{sub}/profile` (never a JWT name claim, never usernames, never `/api/v1/me`; `Player <id8>` fallback) — the title line reads "Signed in as <nickname> · sync status" and the profile name field is read-only when hosted. `GET /api/v1/time` syncs the clock with round-trip adjustment so the daily countdown and day key follow platform time. `POST /api/v1/scores/daily` submits `{dayKey, contentVersion, seed, name, scoreTotal, durationSec, playerId, clientHash, log}` (Bearer-authenticated, graceful offline fallback to casual local boards); `server.js` rebuilds the content from the day key, rejects non-current days (409 `day-not-current`), stale content versions, seed mismatches, malformed logs and bad player ids, replays the log with `DWSession.replay`, requires the recomputed hash to equal `clientHash`, and stores the recomputed score and elapsed time keyed by `playerId` (100 rows per day, 30 posts / 10 s per IP). `GET /api/v1/scores/daily[?day=]` and `/scores/global` feed the Leaderboards screen. The profile document mirrors to one zip+base64 cloud slot at `GET/PUT /api/v1/me/cloud-saves/{slug}` — remote wins on boot (checksum-validated through `DWSession.loadProfileRaw`), saves debounce 2 s and flush on `pagehide`/hidden with keepalive. Achievements stay local (the old `POST /api/v1/achievements/<key>` call had no server route and is gone); activity/presence/telemetry are inert (no launch-token endpoints) with the consent-gated funnel kept in a local buffer. Structured `{"error":…}` and 429 responses surface as recoverable UI text.

**Not used:** realtime sessions, rooms, matchmaking, chat/voice, friends filtering (the `friends=1` query exists in `platform.getBoard` but no UI calls it). Standalone (no token) everything degrades to local casual boards and local achievement flags, per https://wiki.starhermit.com/ conventions for offline-capable solo games.
